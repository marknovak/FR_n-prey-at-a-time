...
order.of.fits <- function(ffr.fits,
  ...
  if (order) {
    if (order.parm == 'Sample size') {
      ...
      foc.parms <-
        unlist(lapply(ffr.fits, function(x)
          x$estimates[[model]][point.est, order.parm, "estimate"]))
      how.to.order <- order(foc.parms)
    }
  } else{
...
plot.coefs <- function(ffr.fits = NULL,
                       model = c('Holling.I',
                                 'Holling.II',
                                 'Holling.n'),
                       parameter = c('attack',
                       ...
                       xlim = NULL,
                       vertLines = NULL,
                       color.factor = c('None', 'Parasite', 'Replacement'),
                       color.vector = NULL,
  ...
  if (is.null(xlim)) {
    ests <-
      unlist(lapply(ffr.fits, function(x)
        x$estimates[[model]][point.est, parameter, "estimate"]))
  ...
  if(is.null(log.axis.base)){
    ...
    xat = outer(1:9, axis.unlink(0:100))
    xlabels = ifelse(axis.link(xat) %% 1 == 0, xat, NA)
    axis(1,
         at = xat, 
         labels = xlabels,
         cex.axis = 0.7,
  ...
  for (i in 1:length(ffr.fits)) {
    ...
    if (!color.factor == 'None') {
      if (color.factor == 'Parasite' & !x$study.info$predator.parasite) {
        col <- 'red'
      }
    ...
    if (length(color.vector) == length(ffr.fits)) {
      # note that this overrides color.factor
      col <- color.vector[i]
    }
